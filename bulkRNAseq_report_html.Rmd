---
title: "Transcriptomic Effect of Adherent Media vs Sphere Media in PDAC"
...
```{r settings}
# Change title
# Change and adapt lines 386-403, 489
...
discarded_samples<-c("ERR978114","ERR978145","ERR978189","ERR978196","ERR978218","ERR978226")
nocpm_threshold<-29 #IMPORTANT PARAMETRE: minimum number of samples with CPM > 1
...
2. Folder with all the plots from Isoform Switch-positive genes.
...
2. **MA Plot**: Showing the expression and statistical significance of every gene on the reads, but with a different representation.

#### Functional Enrichment
1. **Bubble Plot C2 and H**: Showing the top 20 enriched terms on either the *C2* collection or *Hallmark* collection.
2. **Manhattan Plot**: Showing the enriched terms on Gene Ontology, from either Up Regulated genes or Down Regulated genes in the condition `r condition1`.
3. **GSEA**: Gene Set Enrichment Plots from the top enriched terms on the *C2* and *H* collections. 15 of the most upregulated genes, and 15 of the most downregulated genes.

#### Heatmaps
1. **Heatmap_Full**: Heatmap of gene expression across all samples and modules.
2. **Heatmap_Up**: Heatmap of gene expression across all samples but only the module representing the upregulated genes *(overexpressed in `r condition1`)*.
3. **Heatmap_Down**: Heatmap of gene expression across all samples but only the module representing the downregulated genes *(overexpressed in `r condition_control`)*.
...
9. **table8_functionalEnrichment.xlsx**: Taking the log-fold change values from *Table 1* to the *C2* and *Hallmark* databases, the following table of enriched pathways and routes was obtained.
...
## Pseudo-alignment and Automated Script
...
targets <- read.table("studydesign.txt")
...
```{r step 1 and 2 -filtered out samples-, fig.cap="Violin plots of log2 CPM values for each sample. A) Raw reads after mapping. B) Genes with sufficient samples with CPM < 1 were filtered out. C) Normalisation with TMM. D) Tidy Samples with a positive FastQC Result to be used in the downstream analysis"}
...
mydata.df <- mutate(log2.cpm.filtered.norm.df,
                    adh.AVG = (ERR978111+ERR978113+ERR978115+ERR978116+ERR978117+ERR978118+ERR978120+ERR978121+ERR978142+ERR978144+ERR978151+ERR978107+ERR978112+ERR978143+ERR978147+ERR978148+ERR978149)/20, 
                    sph.AVG = (ERR978190+ERR978192+ERR978195+ERR978215+ERR978219+ERR978220+ERR978222+ERR978224+ERR978193+ERR978212+ERR978217+ERR978221+ERR978223+ERR978225+ERR978183+ERR978184+ERR978185+ERR978186+ERR978187+ERR978188+ERR978191+ERR978194+ERR978213+ERR978216)/24,
...
scatter <- ggplot(mydata.df) + 
  aes(x=adh.AVG, y=sph.AVG, color=abs(LogFC), text = paste("Symbol:", geneID)) +
...
```{r datable -only in html, eval=TRUE}
...
save_plot("report/plots/multivariate_analysis/pca.png",pca.plot_condition, base_width = 15, base_height = 7)
...
```{r small multiples, fig.cap="Small Multiples Plot showing the loadings, or contribution, of sample gene to the first 4 principal components."}

ggplotly(small_multiples_plot)
```
...
# MA Plot
```{r MA Plot, fig.cap="The plot is an MA plot colored by False Discovery Rate (FDR). It shows the relationship between average expression level (AveExpr) and log fold change (logFC) for genes or features. Points are colored blue for FDR < 0.01 and red for FDR >= 0.01, aiding in identifying significant changes."}
ma_color <- ifelse(myTopHits.df$adj.P.Val >= 0.01, "red", "blue")
...
ma_plot <- ggplot(myTopHits.df, aes(x = AveExpr, y = logFC,text = paste("Symbol:", geneID))) +
  ...
  ggtitle("MA Plot Coloured by FDR Test>=0.01") +
  ...
  scale_color_manual(values = c("blue", "red"), labels = c("FDR < 0.01", "FDR >= 0.01")) +  # Custom color scale
  theme(
...
# Define file paths
gtf_file_path <- "Homo_sapiens.GRCh38.111.chr_patch_hapl_scaff.gtf.gz"
...
```{r table 3 Isoform Switches only html}
...
```{r example switchplot, fig.cap="Sample of a SwitchPlot. All SwitchPlots generated can be found in their corresponding folder."}
switchPlot(
  mySwitchList,
...
mygostplot_up<-gostplot(gost.res_up, interactive = T, capped = T)
mygostplot_up
```
...
mygostplot_down<-gostplot(gost.res_down, interactive = T, capped = T)
mygostplot_down
```
