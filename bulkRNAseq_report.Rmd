...
dir.create("report/IsoformSwitch", showWarnings = FALSE, recursive = TRUE)
...
```{r step 1 and 2 -filtered out samples-}
...
```{r Isoform Switch Analyzer,results='hide', message=FALSE, warning=FALSE, eval=FALSE}
...
```{r example switchplot, eval=FALSE}
switchPlot(
  mySwitchList,
...
```{r functionalEnrichment part 1 (gostplot for upregulated genes) pdf version}
...
mytable_up_sorted<-dplyr::arrange(gost.res_up$result,p_value)%>%
  ...
  as_tibble()
write.csv(mytable_up_sorted, file = "report/tables/GeneOntology_upRegulated.csv")

save_plot("report/plots/manhattanplotFunctionalEnrichment_Up.svg",mygostplot_up)
save_plot("report/plots/manhattanplotFunctionalEnrichment_Up.png",mygostplot_up)


mygostplot_up
```
```{r functionalEnrichment part 1 (gostplot top 10 for upregulated genes) pdf version}
publish_gostplot(
...
datatable(mytable_up_sorted, 
          extensions = c('KeyTable', "FixedHeader"), 
          caption = 'Table 4: Functional Enrichment of Up Regulated genes in Sph',
          options = list(keys = TRUE, searchHighlight = TRUE, pageLength = 10, lengthMenu = c("10", "25", "50", "100")))
```

```{r functionalEnrichment part 1 (gostplot for Downregulated genes) pdf version}
#NOTE: The idea here is to provide 3 results: a simple gostplot with no labels, a gostplot highlighting the top 10 terms, and a table with all the results.
...
mytable_down_sorted<-dplyr::arrange(gost.res_down$result,p_value)%>%
  ...
  apply(2,as.character)%>%
  as_tibble()
write.csv(mytable_down_sorted, file = "report/tables/GeneOntology_downRegulated.csv")

save_plot("report/plots/manhattanplotFunctionalEnrichment_Down.svg",mygostplot_down)
save_plot("report/plots/manhattanplotFunctionalEnrichment_Down.png",mygostplot_down)


mygostplot_down
```
```{r functionalEnrichment part 1 (gostplot top 10 for downregulated genes) pdf version}
publish_gostplot(
...
datatable(mytable_down_sorted, 
          extensions = c('KeyTable', "FixedHeader"), 
          caption = 'Table 4: Functional Enrichment of Down Regulated genes in Sph',
          options = list(keys = TRUE, searchHighlight = TRUE, pageLength = 10, lengthMenu = c("10", "25", "50", "100")))
```
...
gseaplot2(myGSEA.res, 
          geneSetID = 90, #can choose multiple signatures to overlay in this plot
          pvalue_table = T, #can set this to FALSE for a cleaner plot
          title = myGSEA.res$Description[90]) #can also turn off this title
```

```{r make enrichment plots for both phenotypes, results='hide'}
for (i in 1:15){
  name<-paste0("report/plots/GSEAplot_SphEnriched",i,".svg")
  save_plot(name, gseaplot2(myGSEA.res, 
          ...
          title = myGSEA.res$Description[i]))
}

for (i in 1:15){
  name<-paste0("report/plots/GSEAplot_AdhEnriched",i,".svg")
  save_plot(name, gseaplot2(myGSEA.res, 
          geneSetID = which(myGSEA.df$NES<0)[i], #can choose multiple signatures to overlay in this plot
          pvalue_table = F, #can set this to FALSE for a cleaner plot
          title = myGSEA.res$Description[i]))
...
myGSEA.df <- myGSEA.df %>%
  mutate(phenotype = case_when(
    ...
    NES < 0 ~ "adh"))
abs(myGSEA.df$NES)
write.csv(myGSEA.df, file = "report/tables/functionalEnrichment.csv")
```
...
```{r functionalEnrichment part 5 (bubble plot)}
...
bubbleplot<-ggplot(myGSEA.df[top$ix[1:20],], aes(x=phenotype, y=ID)) + 
  ...
  theme_bw()
save_plot("report/plots/bubbleplotEnichment.svg", bubbleplot)
save_plot("report/plots/bubbleplotEnichment.png", bubbleplot)
bubbleplot
```
