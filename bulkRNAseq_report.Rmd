...
```{r multivariate part 1 (data table)}
...
```{r  multivariate part 2 (PCA plot), warning=FALSE, message=FALSE}
media <- targets$media
media <- factor(media)
...
```{r diffGenes part 1 (volcano plot), warning=FALSE}
...
input_pfam<-read_lines("report/IsoformSwitchAnalyzer/isoformSwitchAnalyzeR_isoform_AA_complete.fasta")

output_pfam <- pfamscan(
  fasta_str = input_pfam,
  email = "andresgordoortiz@gmail.com",
...
png(file = "report/plots/heamaps/heatmap_downRegulated.png")
heatmap.2(myModule_down, 
          Rowv=as.dendrogram(hrsub_down), 
          Colv=as.dendrogram(clustColumns), 
          labRow = NA,
...
```{r functionalEnrichment part 5 (bubble plot), fig.align='left'}
